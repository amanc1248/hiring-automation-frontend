...
const mockWorkflowExecutions: WorkflowExecution[] = [
  {
    ...
    nextStep: mockWorkflows[0].steps[3], // Task Review
    startedAt: '2024-01-15T10:00:00Z',
...
export const workflowService = {
  ...
  async getWorkflowTemplate(templateId: string): Promise<WorkflowTemplate | null> {
    await delay(300)
    return mockWorkflowTemplates.find(template => template.id === templateId) || null
  },

  // Workflows
  async getWorkflows(companyId: string): Promise<Workflow[]> {
    await delay(500)
    return mockWorkflows.filter(workflow => workflow.companyId === companyId)
  },

  async getWorkflow(workflowId: string): Promise<Workflow | null> {
    await delay(300)
    return mockWorkflows.find(workflow => workflow.id === workflowId) || null
  },

  async getWorkflowByJob(jobId: string): Promise<Workflow | null> {
    await delay(300)
    return mockWorkflows.find(workflow => workflow.jobId === jobId) || null
  },
  ...
  async updateWorkflow(workflowId: string, updates: WorkflowUpdateData): Promise<Workflow | null> {
    await delay(600)
    const workflowIndex = mockWorkflows.findIndex(workflow => workflow.id === workflowId)
